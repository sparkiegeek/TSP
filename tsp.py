...
def swapped_cities(tour):
    '''generator to create all possible variations where two cities
    have been swapped'''
    for i, j in all_pairs(len(tour)):
        if i < j:
            copy = list(tour)
...
def cartesian_matrix(coords):
    '''create a distance matrix for the city coords that uses straight
    line distance'''
    matrix = {}
...
def run_hillclimb(init_function, move_operator,
...
def run_anneal(init_function, move_operator, objective_function,
               max_iterations, start_temp, alpha):
    if start_temp is None or alpha is None:
        usage()
        print "missing --cooling start_temp:alpha for annealing"
        sys.exit(1)
    from sa import anneal
    iterations, score, best = anneal(init_function, move_operator,
                                     objective_function, max_iterations,
                                     start_temp, alpha)
    return iterations, score, best


def run_evolve(init_function, move_operator, objective_function,
               max_iterations, pop_size, tournament_size):
    if pop_size is None:
        usage()
        print "missing --popsize size for evolve"
        sys.exit(1)
    from ga import evolve
    iterations, score, best = evolve(init_function, move_operator,
                                     objective_function, max_iterations,
                                     recombine, pop_size, tournament_size)
    return iterations, score, best


def usage():
    #TODO: optparse goodness
    print ("usage: python %s [-o <output image file>] [-v] "
           "[-m reversed_sections|swapped_cities] "
           "-n <max iterations> [-a hillclimb|anneal] "
           "[--cooling start_temp:alpha] "
           "<city file>" % sys.argv[0]


def main():
    try:
        options, args = getopt.getopt(
            sys.argv[1:], "ho:vm:n:a:",
            ["cooling=", "popsize=", "tournament="])
        ...
        if option == '-v':
            ...
            if arg == 'hillclimb':
                ...
                def run_evolve_with_pop_size(init_function, move_operator,
                                             objective_function,
                                             max_iterations):
                    return run_evolve(init_function, move_operator,
                                      objective_function, max_iterations,
