...
class Individual(object):
    ...
    def __init__(self, solution, objective_function,
                 move_operator, recombine_operator):
    ...
    def _new_from_solution(self, solution):
        return Individual(
            solution, self.objective_function, self.move_operator,
            self.recombine_operator)

    def breed(self, parent):
        if random.random() < 1: # TODO: Isn't this guaranteed?
            solution = self.recombine_operator(self.solution, parent.solution)
            return self._new_from_solution(solution)
        return self

    def mutate(self):
        if random.random() < 1: # TOOD: Isn't this guaranteed?
...
def steady_state(init_function, move_operator, objective_function,
                 max_evaluations, recombine_operator, pop_size,
                 breed_size=50, replace_size=2):
    objective_function = ObjectiveFunction(objective_function)

    insert = lambda population, child: \
        replace_worst_if_better(population, child, replace_size)

    population = set([Individual(init_function(), objective_function,
                                 move_operator, recombine_operator)
    ...
    return (objective_function.num_evaluations,
...
def generational(init_function, move_operator, objective_function,
                 max_evaluations, recombine_operator, pop_size):
    objective_function=ObjectiveFunction(objective_function)
...
def generational2(init_function, move_operator,
                  objective_function, max_evaluations,
                  recombine_operator, pop_size):
    objective_function=ObjectiveFunction(objective_function)

    population = [Individual(init_function(), objective_function,
                             move_operator, recombine_operator)
                  for i in xrange(pop_size)]
    population.sort()
    next_population = []
